...
var (
	// ErrNotFound is used when a specific User is requested but does not exists.
...
func (u User) Query(ctx context.Context, traceID string, pageNumber int, rowsPerPage int) ([]Info, error) {
	...
	users := []Info{}
	if err := u.db.SelectContext(ctx, &users, q, pageNumber, rowsPerPage); err != nil {
		if err == ErrNotFound {
